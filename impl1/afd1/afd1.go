...
func main() {
	...
	q0 := state{
		name:      "q0",
		transFunc: map[string]string{"0": "q0", "1": "q1"},
	}

	q1 := state{
		name:      "q1",
		transFunc: map[string]string{"0": "q2"},
	}

	q2 := state{
		name:      "q2",
		transFunc: map[string]string{"0": "q0"},
	}
	...
	automata := automata{
		...
		initState:   q0,
		finalStates: map[string]state{"q0": q0},
	}
...
func (a *automata) transLogic(inputs []string) {
	...
	if len(inputs) == 0 { // caso a entrada seja vazia, ela é aceita
